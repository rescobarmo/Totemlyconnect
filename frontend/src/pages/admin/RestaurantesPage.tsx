...
export default function RestaurantesPage() {
  ...
  const openEdit = (r: Restaurante) => {
    setEditId(r.id);
    setForm({ nombre: r.nombre, direccion: r.direccion || "", telefono: r.telefono || "" });
    setModal(true);
  };
  ...
  const save = async () => {
    try {
      if (editId) {
        await api.put(`/restaurantes/${editId}`, form);
      } else {
        await api.post("/restaurantes", form);
      }
      setModal(false);
      load();
    } catch (err: any) {
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-700">
                ...
                <th className="text-left px-4 py-3 text-slate-400 text-sm">Teléfono</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={r.id} className="border-b border-slate-700/50">
                  ...
                  <td className="px-4 py-3 text-slate-300">{r.telefono || "—"}</td>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setModal(false)}>
          <div className="bg-slate-800 rounded-2xl p-6 w-full max-w-md" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={save} disabled={!form.nombre} className="flex-1 py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-800 text-white font-medium rounded-lg">Guardar</button>
            </div>
          </div>
        </div>
      )}
